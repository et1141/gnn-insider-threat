...
# Raw CERT data is multi-GB and DVC-tracked, so it is absent in CI and on
# fresh clones. This test validates that the DuckDB SQL pipeline matches the
# reference Python implementation on the real CSVs — only meaningful when the
# data is present, hence the skip guard rather than a hard failure.
_EXTRACT_DIR = get_project_root() / "data/raw/cmu_cert_r5.2/r5.2"
pytestmark = pytest.mark.skipif(
    not (_EXTRACT_DIR / "logon.csv").exists(),
    reason="raw CERT r5.2 data not present (DVC-tracked); skipping real-data equivalence check",
)
